...
// Enhanced extraction function that extracts image-relevant details
function extractImageDetails(data: any, inputText: string) {
  console.log("🎨 === EXTRACTING IMAGE DETAILS ===")

  // Extract brand from offer name or text
  let extractedBrand = ""
  if (data.offerName) {
    extractedBrand = data.offerName.split(" ")[0] // First word is usually brand
  }

  // Extract products from rules or text
  ...
  // If no products in rules, try to extract from text
  ...
  // Extract category based on products or brand
...
// OpenAI extraction via API route
async function extractWithOpenAI(text: string) {
  ...
  try {
    ...
    if (!data.success) {
      throw new Error(`OpenAI extraction failed: ${data.error}`)
    }
    ...
    console.log("🤖 AI Generated headline:", data.data.offerHeadline)
    console.log("🤖 AI Generated bodyline:", data.data.offerBodyline)
    console.log("🤖 Rules from API:", data.data.offerRules)
    console.log("🤖 Rules count:", Object.keys(data.data.offerRules || {}).length)
    console.log("🤖 Start date:", data.data.offerStartDate)
    console.log("🤖 End date:", data.data.offerEndDate)
...
export default function ChatModal({ isOpen, onClose, onExtractData }: ChatModalProps) {
  ...
  // Check OpenAI availability when modal opens
  useEffect(() => {
    if (isOpen) {
      checkOpenAIStatus()
    }
  ...
  const checkOpenAIStatus = async () => {
    try {
      ...
      const testResponse = await fetch("/api/openai-extract", {
        ...
        body: JSON.stringify({ text: "Test offer: 10% off shoes" }),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log("🎯 Start date:", result.data.offerStartDate)
        console.log("🎯 End date:", result.data.offerEndDate)

        // Extract image-specific details from LLM data
        const imageDetails = extractImageDetails(result.data, inputText)

        // Generate enhanced thumbnail using Canvas with LLM details
        console.log("🎨 === ENHANCED CANVAS IMAGE GENERATION WITH LLM DATA ===")
        ...
        const offerImage = generateEnhancedThumbnail({
          ...
          width: 120,
          height: 120,
          // Pass LLM-extracted details for better image generation
        ...
        console.log("🎨 Enhanced Canvas generation time:", imageTime + "ms")
        console.log("🎨 Used brand:", imageDetails.extractedBrand)
        console.log("🎨 Used products:", imageDetails.extractedProducts)
        console.log("🎨 Used category:", imageDetails.extractedCategory)
        ...
        console.log("🎯 Final rules:", extractedData.offerRules)
...
• Headline: ${extractedData.offerHeadline || "Not found"} 🤖
• Bodyline: ${extractedData.offerBodyline || "Not found"} 🤖
...
✅ Form populated with AI-generated copy and brand-specific image!`
        ...
        <CardContent className="flex-1 flex flex-col p-6">
              ...
              <SelectContent>
                ...
                <SelectItem value="regex">
                  <div className="flex items-center">
                    ...
                    <Palette className="w-4 h-4 mr-2 text-purple-600" />⚡ Regex + Smart Canvas
                  </div>
                </SelectItem>
              </SelectContent>
            </Select>
          ...
          <div className="mb-4">
            <Alert className="border-blue-200 bg-blue-50">
              ...
              <AlertDescription>
                <strong className="text-blue-700">🎨 Image Generation Options:</strong>
                <br />
                <span className="text-sm text-blue-600">
                  <strong>Current:</strong> Smart Canvas (precise control, brand-specific, instant, free)
                </span>
                <br />
                <span className="text-sm text-blue-600">
                  <strong>Alternative:</strong> OpenAI DALL-E (creative, costs ~$0.02/image, slower)
                </span>
                <br />
                <span className="text-sm text-blue-600">
                  💡 Canvas gives us exact control over text, branding, and offer details!
                </span>
              </AlertDescription>
            </Alert>
          </div>
          ...
          <div className="mb-4">
            ...
            {openaiStatus === "error" && (
              <Alert className="border-red-200 bg-red-50">
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  <strong className="text-red-700">❌ OpenAI API Issue</strong>
                  <br />
                  <span className="text-sm text-red-600">{statusDetails}</span>
                  <br />
                  <div className="mt-2 space-y-1 text-sm">
                    <div>• Will attempt OpenAI first, then fallback to Enhanced Regex + Smart Canvas</div>
                    <div>
          ...
          <div className="flex-1 mb-4 space-y-4 overflow-y-auto max-h-60">
            {chatHistory.length === 0 && (
              <div className="text-center text-gray-500 py-8">
                ...
                <p className="text-lg font-medium">OpenAI GPT-4o + Smart Canvas</p>
                <p className="text-sm mt-2">🤖 AI-generated headlines/bodylines + 🎨 Brand-specific images</p>
              </div>
            )}
          ...
          <div className="flex-shrink-0 space-y-4">
            ...
            <div className="flex space-x-2">
              ...
              >
                ...
                {isProcessing ? "Processing..." : "Extract with Smart Canvas"}
              </Button>
